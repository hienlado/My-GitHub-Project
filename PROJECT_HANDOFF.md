...
- **⚠ 2 BẪY HIỆU NĂNG/ĐỒNG BỘ đã sửa:** (1) `loadSheetFramesIfNeeded()` chỉ *khởi chạy* coroutine rồi trả về ⇒ gọi xong đọc `_sheetFrames.value` vẫn RỖNG ⇒ `loadAdjacentSheets` luôn thoát sớm, sơ hoạ không bao giờ có tờ giáp biên. Đã thêm **`suspend ensureSheetFrames()`** chờ nạp xong. (2) Overlay khung tờ vẽ **TẤT CẢ ~4600 tờ = ~9200 overlay** ⇒ lag nặng khi bật/tắt và khi kéo bản đồ. Nay **lọc theo khung nhìn** (nới 30%) + trần `MAX_SHEET_FRAMES = 250`.
- **⚠ BẪY LỌC THỬA GIÁP BIÊN (mất 3 lượt mới ra):** `pickNeighbours` lọc theo bbox rồi `.take(maxCount)` **theo thứ tự danh sách** ⇒ vớ phải thửa xa nằm đầu tờ, bỏ mất thửa ôm sát thửa chính (nằm cuối danh sách). Phải **tính KHE HỞ giữa 2 bbox (`gap`, 0 = chạm nhau) rồi `sortedBy { gap }` TRƯỚC khi cắt**. Ngoài ra bán kính tìm phải **tuyệt đối theo mét** (`minExpandM=120m`, `searchM=250m`), không nới theo tỉ lệ kích thước thửa — thửa nhỏ 20 m chỉ dò được 30 m nên trượt hết tờ kề.
...
- **GPS điện thoại dự phòng:** `core/gnss/PhoneGpsFallback.kt` + `GnssDataManager.startPhoneGpsFallback()` — khi **CHƯA nối máy thu** vẫn có vị trí để định vị/tra tờ. Hiển thị **"GPS ĐT"** (màu tím) qua `GnssStatus.isPhoneGps`, **không** phải "SINGLE". Có NMEA thật là tự tắt. **3 BẪY ĐÃ SỬA:** (1) `startNmeaWatchdog` mỗi giây hạ `fixQuality=0` → xoá sạch vị trí dự phòng ⇒ phải `continue` khi `phoneGps.running`; (2) `coordSettings` chỉ nạp trong `startNmeaProcessing()` nên chưa nối máy thì là mặc định ⇒ nạp độc lập trong `startPhoneGpsFallback()`; (3) thiếu quyền vị trí thì lớp dự phòng **im lặng bỏ qua** ⇒ MapScreen phải xin `ACCESS_FINE_LOCATION` trước.
